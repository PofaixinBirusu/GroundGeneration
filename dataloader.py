...
import torch
import models.backbone_kpconv.cpp_wrappers.cpp_subsampling.grid_subsampling as cpp_subsampling
import models.backbone_kpconv.cpp_wrappers.cpp_neighbors.radius_neighbors as cpp_neighbors
import time
...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    for ind, (inp, inp_feats, ground_mask) in enumerate(list_data):
        batched_points_list.append(inp)
        batched_features_list.append(inp_feats)
        batched_lengths_list.append(len(inp))
        batched_ground_list.append(torch.from_numpy(ground_mask))
    ...
    dict_inputs = {
        ...
        'ground_mask': batched_ground_list
    }
